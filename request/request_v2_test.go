...
import (
	...
	"github.com/radekg/kafka-protocol-go/sasl"
...
func TestEncodeDecodeRequestV2(t *testing.T) {
	...
	err = common.Decode(reqBytes[0:8], requestKeyVersion) // Size => int32 + ApiKey => int16 + ApiVersion => int16
	a.Nil(err)
	...
	body := &sasl.SaslAuthenticateRequestV2{}
	request := &RequestV2{Body: body}
	err = common.Decode(reqBytes[4:], request)
	a.Nil(err)
